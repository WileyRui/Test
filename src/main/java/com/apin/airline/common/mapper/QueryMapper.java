...
public interface QueryMapper extends Mapper {
    ...
    @Select("select c.id airlineId,sum(a.seat_count) AS total," +
            "sum(b.seat_count) remainCount, min(b.adult_price) basePrice,e.days+1 days,DATE_ADD(b.flight_date,INTERVAL e.days DAY) retDate,c.flight_type flightType,sum(CONV(left(b.id,1),16,10)%9+1 ) soldCount FROM mbs_airline a  " +
