    ...
    public String searchDayAirlines(SearchDayAirlinesDto searchAirlineDto){
        StringBuffer sqlBuffer = new StringBuffer(
                "SELECT                                                                                                                                                                                              "
                        ...
                        +"  (SELECT mos.flight_id, sum( mos.adult_count + mos.child_count ) saldCount FROM mbt_order mo, mbt_order_supplier mos WHERE mo.id = mos.order_id AND mo.order_status IN (4, 5, 6, 7) AND mo.is_invalid = 0 GROUP BY mos.flight_id) seatMain on mbsaf.id = seatMain.flight_id   "
                        +"WHERE                                                                                                                                                                                               "
